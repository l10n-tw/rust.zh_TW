lint_array_into_iter =
    這個方法呼叫會（因向後相容性）解釋為 `<&{$target} as IntoIterator>::into_iter`，但會在 Rust 2021 解釋為 `<{$target} as IntoIterator>::into_iter`
    .use_iter_suggestion = 請改用 `.iter()`（而非 `.into_iter()`）防止歧義
    .remove_into_iter_suggestion = 或移除 `.into_iter()` 以改成依值疊代
    .use_explicit_into_iter_suggestion =
        或改用 `IntoIterator::into_iter(..)`（而非 `.into_iter()`）以顯式依值疊代

lint_enum_intrinsics_mem_discriminant =
    使用非枚舉類型呼叫 `mem::discriminant` 時，其回傳值未指定
    .note = 傳入 `discriminant` 之引數應為指向枚舉 (enum) 的引用，卻傳入了指向 `{$ty_param}`——不是枚舉——的引用。

lint_enum_intrinsics_mem_variant =
    使用非枚舉類型呼叫 `mem::variant_count` 時，其回傳值未指定
    .note = `variant_count` 的類型參數應為枚舉 (enum)，但呼叫者卻以 `{$ty_param}`——不是枚舉的——類型，實例化本函數。

lint_expectation = 尚未滿足 lint 的期望值
    .note = `unfulfilled_lint_expectations` lint 不能被期望，且會永遠產出此訊息
